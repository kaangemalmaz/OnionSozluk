...
namespace OnionSozluk.Infrastructure.Persistence.Repositories
{
        ...
        where TEntity : BaseEntity
    {
        ...
        {
            // bir entityi update edebilmek için önce get edilir. sonra prop değişir ve update edilir.
            // Bir varlığa güncelleme yapmayı planlıyor olsanız bile, varlığın çok sayıda özelliği (db sütunları) varsa ve yalnızca birkaçını güncellemek istiyorsanız, bir DbSet.Attach(entity). güncellenmesi gereken. kısmı günceller. yani senin yerine get alıyor sonrasında gerekli stunları güncelliyor gibi düşünebilirsin.
        ...
        {
            //Detached - Entity bir context tarafından izlenmemektedir.Yani Entity ile Context arasında aktif bir bağlantı yoktur.

            if (dbContext.Entry(entity).State == EntityState.Detached) //Concurrency için 
            {
                this.entity.Attach(entity); // eğer izlenmiorsa izle demektir. yani aslında attach = tracking yani veritabanı ile ilişkilendir.
            }
        ...
        public virtual async Task<TEntity> GetByIdAsync(Guid id, bool noTracking = true, params Expression<Func<TEntity, object>>[] includes)
        {
            ...
            foreach (Expression<Func<TEntity, object>> include in includes)
            {
                // find ile bulunan bir yapı için include
        ...
        #region Bulk Methods
        // bulk toplu data işleme zamanında yapılan işlemlerdir.
